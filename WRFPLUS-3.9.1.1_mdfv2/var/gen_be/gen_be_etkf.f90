...
#include "netcdf.inc"
   ...
   real, pointer         :: ens_mean(:)               ! Variable ensemble average.
...
!-----------------------------------------------------------------------------------------
   write(stdout,'(/a)')' [3] Set up arrays using input ensemble average forecast'
!-----------------------------------------------------------------------------------------
!
! OPEN THE ensemble average DATA FILE
...
!
! PRINT A PRIORI SPATIALLY AVERAGED ensemble average AND SQUARE ROOT OF 
...
! CALCULATE THE POSTERIORI STANDARD DEVIATION
     ...
     write(stdout,'(5a)')'   v', ' Variable  ', '    ensemble average', &
